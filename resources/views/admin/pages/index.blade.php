...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
    <h1 class="text-lg font-semibold text-white">{{ __('admin.pages') }}</h1>

    <a href="{{ route('admin.pages.create') }}" class="inline-flex items-center gap-2 px-3 py-1.5 bg-sky-600 hover:bg-sky-700 text-white text-sm font-medium rounded-md transition-colors">
...
@if ($pages->isEmpty())
    <div class="text-center py-20 text-gray-600">
        ...
        <p class="text-sm font-medium text-gray-500">{{ __('admin.no_pages_yet') }}</p>
        <p class="text-sm text-gray-600 mt-1">{{ __('admin.no_pages_hint') }}</p>
        <a href="{{ route('admin.pages.create') }}" class="mt-4 inline-flex items-center gap-2 px-3 py-1.5 bg-sky-600 hover:bg-sky-700 text-white text-sm font-medium rounded-md transition-colors">
            {{ __('admin.new_page') }}
        </a>
    </div>
@else
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-white/[0.06]">
                    ...
                    <th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3 hidden md:table-cell">{{ __('admin.col_template') }}</th>
            ...
            <tbody class="divide-y divide-white/[0.04]">
                @foreach ($pages as $page)
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        <td class="px-4 py-3">
                            <a href="{{ route('admin.pages.edit', $page) }}" class="text-sm text-white hover:text-sky-400 transition-colors">
                                {{ $page->title }}
                            </a>
                        </td>
                        ...
                        <td class="px-4 py-3 hidden md:table-cell">
                            <div class="flex items-center gap-1">
                                @foreach(($locales ?? collect()) as $locale)
                                    @php
                                        $state = $page->derivedTranslationState($locale->code);
                                        $translation = $page->translations->firstWhere('locale', $locale->code);
                                        $badgeColor = $state === 'published'
                                            ? 'bg-sky-500/10 text-sky-400'
                                            : ($state === 'missing' ? 'bg-white/[0.04] text-gray-600' : 'bg-yellow-500/10 text-yellow-400');
                                    @endphp
                                    @if($translation)
                                        <a href="{{ route('admin.pages.edit', $translation) }}"
                                    ...
                                    @elseif($page->isTranslationSchemaReady())
                                        <form method="POST" action="{{ route('admin.pages.translate', [$page, $locale->code]) }}" class="inline-flex">
                                            @csrf
                                            <button type="submit"
                                    ...
                                    @else
                                        <span title="{{ __('admin.translation_not_supported') }}"
                                              class="inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-medium {{ $badgeColor }}">
                                            +{{ strtoupper($locale->code) }}
                                        </span>
                                    @endif
                                @endforeach
                        ...
                        <td class="px-4 py-3 hidden md:table-cell">
                            <span class="text-xs text-gray-500">{{ config('cms.templates.' . $page->template_key . '.name', $page->template_key) }}</span>
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @if ($page->isPublished())
                                    <a href="{{ $page->url() }}" target="_blank" class="text-gray-600 hover:text-gray-400 transition-colors" title="{{ __('admin.action_view') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                        </svg>
                                    </a>
                                @endif
                                <a href="{{ route('admin.pages.edit', $page) }}" class="text-gray-600 hover:text-gray-400 transition-colors" title="{{ __('admin.action_edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form method="POST" action="{{ route('admin.pages.destroy', $page) }}" onsubmit="return confirm('{{ __('admin.confirm_delete_page') }}')" class="flex items-center">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center text-gray-600 hover:text-red-400 transition-colors" title="{{ __('admin.btn_delete_page') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
