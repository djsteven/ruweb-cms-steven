...
@section('title', __('admin.new_post_title'))

@section('content')
<div class="mb-6 flex items-start justify-between gap-4">
    <div>
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
            <a href="{{ route('admin.posts.index') }}" class="hover:text-gray-300 transition-colors">{{ __('admin.posts') }}</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-gray-400">{{ __('admin.new_post_title') }}</span>
        </div>
        <h1 class="text-lg font-semibold text-white">{{ __('admin.new_post_title') }}</h1>
    </div>
...
<script>
    ...
    ['publish-btn', 'publish-btn-mobile'].forEach(id => {
        ...
        if (el) el.addEventListener('click', () => submitWithStatus('published'));
    });
    ['save-draft-btn', 'save-draft-btn-mobile'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.addEventListener('click', () => submitWithStatus('draft'));
    });
</script>
@endpush
@endsection
